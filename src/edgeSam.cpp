...
struct SamModel {
  ...
  SamModel(const Sam::Parameter& param) {
    ...
    sessionPre = std::make_unique<Ort::Session>(env, wpreModelPath.c_str(), Ort::SessionOptions{ nullptr });
    ...
    sessionSam = std::make_unique<Ort::Session>(env, wsamModelPath.c_str(), Ort::SessionOptions{ nullptr });
  ...
  bool loadImage(const cv::Mat& image) {
    ...
    inputTensorValuesFloat.resize(inputShapePre[0] * inputShapePre[1] * inputShapePre[2] * inputShapePre[3]);                                                 
    for (int i = 0; i < inputShapePre[2]; i++) {                                                
      for (int j = 0; j < inputShapePre[3]; j++) {                                              
        ...
        inputTensorValuesFloat[inputShapePre[2] * inputShapePre[3] + i * inputShapePre[3] + j] = image.at<cv::Vec3b>(i, j)[1];                                                       
        inputTensorValuesFloat[2 * inputShapePre[2] * inputShapePre[3] + i * inputShapePre[3] + j] = image.at<cv::Vec3b>(i, j)[0];                                                       
      }                                                                                         
    }    
    ...
    auto inputTensor = Ort::Value::CreateTensor<float>(memoryInfo, inputTensorValuesFloat.data(), inputTensorValuesFloat.size(), 
  ...
  void getMask(const std::list<cv::Point>& points, const std::list<cv::Point>& negativePoints,
    ...
    if (outputMaskSam.type() != CV_8UC1 || outputMaskSam.size() != cv::Size(inputShapePre[3], inputShapePre[2])) {
