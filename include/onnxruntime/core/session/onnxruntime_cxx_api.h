...
//
// The C++ API simplifies usage by returning values directly instead of error codes, throwing
// exceptions on errors and automatically releasing resources in the destructors.
//
// Each of the C++ wrapper classes holds only a pointer to the C internal object. Treat them like
// smart pointers. To create an empty object, pass 'nullptr' to the constructor (for example, Env
// e{nullptr};).
//
// Only move assignment between objects is allowed, there are no copy constructors. Some objects
// have explicit 'Clone' methods for this purpose.
...
#ifdef ORT_NO_EXCEPTIONS
// The #ifndef is for the very special case where the user of this library wants to define their own
// way of handling errors. NOTE: This header expects control flow to not continue after calling
// ORT_CXX_API_THROW
#ifndef ORT_CXX_API_THROW
#define ORT_CXX_API_THROW(string, code)                                \
    do {                                                               \
        std::cerr << Ort::Exception(string, code).what() << std::endl; \
        abort();                                                       \
    } while (false)
#endif
#else
...
// This is used internally by the C++ API. This class holds the global variable that points to the
// OrtApi, it's in a template so that we can define a global variable in a header and make it
...
// If macro ORT_API_MANUAL_INIT is defined, no static initialization will be performed. Instead,
// user must call InitApi() before using it.
template <typename T>
...
#pragma warning(push)
// "Global initializer calls a non-constexpr function." Therefore you can't use ORT APIs in the
// other global initializers. Please define ORT_API_MANUAL_INIT if it conerns you.
...
/// This is a C++ wrapper for OrtApi::GetAvailableProviders() and returns a vector of strings
/// representing the available execution providers.
std::vector<std::string> GetAvailableProviders();

// This is used internally by the C++ API. This macro is to make it easy to generate overloaded
// methods for all of the various OrtRelease* functions for every Ort* type This can't be done in
// the C API since C doesn't have function overloading.
 ...
 *                                                dims.data(), dims.size(),
 * ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT16); \endcode
 *
 * Here is another example, a little bit more elaborate. Let's assume that you use your own float16
 * type and you want to use a templated version of the API above so the type is automatically set
 * based on your type. You will need to supply an extra template specialization.
 *
 * \code{.unparsed}
 * namespace yours { struct half {}; } // assume this is your type, define this:
 * namespace Ort {
 * template<>
 * struct TypeToTensorType<yours::half> { static constexpr ONNXTensorElementDataType type =
 * ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT16; }; } //namespace Ort
 *
 ...
 * There is a secondary class 'Unowned<T>' that is used to prevent deletion on destruction (Used for
 * return types that are not owned by the caller)
 *
 */
template <typename T>
struct Base {
    ...
    Base() = default;
    Base(T* p) : p_{p} {
        if (!p) ORT_CXX_API_THROW("Allocation failure", ORT_FAIL);
    }
    ~Base() { OrtRelease(p_); }
...
protected:
    ...
    template <typename>
    friend struct Unowned;  // This friend line is needed to keep the centos C++ compiler from
                            // giving an error
};

/** \brief Wraps an object that inherits from Ort::Base and stops it from deleting the contained
...
struct Env : Base<OrtEnv> {
    ...
    Env(OrtLoggingLevel logging_level, const char* logid, OrtLoggingFunction logging_function,
        void* logger_param);
    ...
    Env(const OrtThreadingOptions* tp_options, OrtLoggingFunction logging_function,
        void* logger_param, OrtLoggingLevel logging_level = ORT_LOGGING_LEVEL_WARNING,
    ...
    Env& CreateAndRegisterAllocator(
        ...
        const OrtArenaCfg* arena_cfg);  ///< Wraps OrtApi::CreateAndRegisterAllocator
};
...
struct RunOptions : Base<OrtRunOptions> {
    ...
    RunOptions& AddConfigEntry(const char* config_key,
                               const char* config_value);  ///< Wraps OrtApi::AddRunConfigEntry

    /** \brief Terminates all currently executing Session::Run calls that were made using this
     * RunOptions instance
     *
     * If a currently executing session needs to be force terminated, this can be called from
     * another thread to force it to fail with an error Wraps OrtApi::RunOptionsSetTerminate
     */
    RunOptions& SetTerminate();

    /** \brief Clears the terminate flag so this RunOptions instance can be used in a new
     * Session::Run call without it instantly terminating
     *
...
struct SessionOptions : Base<OrtSessionOptions> {
    ...
    explicit SessionOptions(OrtSessionOptions* p)
        : Base<OrtSessionOptions>{p} {}  ///< Used for interop with the C API
    ...
    SessionOptions& SetIntraOpNumThreads(
        int intra_op_num_threads);  ///< Wraps OrtApi::SetIntraOpNumThreads
    SessionOptions& SetInterOpNumThreads(
        int inter_op_num_threads);  ///< Wraps OrtApi::SetInterOpNumThreads
    SessionOptions& SetGraphOptimizationLevel(
        GraphOptimizationLevel
            graph_optimization_level);  ///< Wraps OrtApi::SetSessionGraphOptimizationLevel
    ...
    SessionOptions& SetOptimizedModelFilePath(
        const ORTCHAR_T* optimized_model_file);  ///< Wraps OrtApi::SetOptimizedModelFilePath

    SessionOptions& EnableProfiling(
        const ORTCHAR_T* profile_file_prefix);  ///< Wraps OrtApi::EnableProfiling
    ...
    SessionOptions& SetExecutionMode(
        ExecutionMode execution_mode);  ///< Wraps OrtApi::SetSessionExecutionMode
    ...
    SessionOptions& AddConfigEntry(
        const char* config_key, const char* config_value);  ///< Wraps OrtApi::AddSessionConfigEntry
    SessionOptions& AddInitializer(const char* name,
                                   const OrtValue* ort_val);  ///< Wraps OrtApi::AddInitializer
    SessionOptions& AddExternalInitializers(
        const std::vector<std::string>& names,
        const std::vector<Value>& ort_values);  ///< Wraps OrtApi::AddExternalInitializers

    SessionOptions& AppendExecutionProvider_CUDA(
        const OrtCUDAProviderOptions&
            provider_options);  ///< Wraps OrtApi::SessionOptionsAppendExecutionProvider_CUDA
    SessionOptions& AppendExecutionProvider_CUDA_V2(
        const OrtCUDAProviderOptionsV2&
            provider_options);  ///< Wraps OrtApi::SessionOptionsAppendExecutionProvider_CUDA_V2
    SessionOptions& AppendExecutionProvider_ROCM(
        const OrtROCMProviderOptions&
            provider_options);  ///< Wraps OrtApi::SessionOptionsAppendExecutionProvider_ROCM
    SessionOptions& AppendExecutionProvider_OpenVINO(
        const OrtOpenVINOProviderOptions&
            provider_options);  ///< Wraps OrtApi::SessionOptionsAppendExecutionProvider_OpenVINO
    SessionOptions& AppendExecutionProvider_TensorRT(
        const OrtTensorRTProviderOptions&
            provider_options);  ///< Wraps OrtApi::SessionOptionsAppendExecutionProvider_TensorRT
    ...
    SessionOptions& AppendExecutionProvider_MIGraphX(
        const OrtMIGraphXProviderOptions&
            provider_options);  ///< Wraps OrtApi::SessionOptionsAppendExecutionProvider_MIGraphX
    ...
    SessionOptions& SetCustomCreateThreadFn(
        OrtCustomCreateThreadFn
            ort_custom_create_thread_fn);  ///< Wraps OrtApi::SessionOptionsSetCustomCreateThreadFn
    SessionOptions& SetCustomThreadCreationOptions(
        void*
            ort_custom_thread_creation_options);  ///< Wraps
                                                  ///< OrtApi::SessionOptionsSetCustomThreadCreationOptions
    SessionOptions& SetCustomJoinThreadFn(
        OrtCustomJoinThreadFn
            ort_custom_join_thread_fn);  ///< Wraps OrtApi::SessionOptionsSetCustomJoinThreadFn
};
...
struct ModelMetadata : Base<OrtModelMetadata> {
    ...
    char* GetProducerName(
        OrtAllocator* allocator) const;  ///< Wraps OrtApi::ModelMetadataGetProducerName
    ...
    AllocatedStringPtr GetGraphNameAllocated(
        OrtAllocator* allocator) const;  ///< Wraps OrtApi::ModelMetadataGetGraphName
    ...
    AllocatedStringPtr GetDomainAllocated(
        OrtAllocator* allocator) const;  ///< Wraps OrtApi::ModelMetadataGetDomain
     ...
     */
    char* GetDescription(
        OrtAllocator* allocator) const;  ///< Wraps OrtApi::ModelMetadataGetDescription
     ...
     * by the specified allocator and is often leaked. Not exception safe.
     */
    char** GetCustomMetadataMapKeys(OrtAllocator* allocator, _Out_ int64_t& num_keys)
        const;  ///< Wraps OrtApi::ModelMetadataGetCustomMetadataMapKeys

    std::vector<AllocatedStringPtr> GetCustomMetadataMapKeysAllocated(
        OrtAllocator* allocator) const;  ///< Wraps OrtApi::ModelMetadataGetCustomMetadataMapKeys
     ...
     * by the specified allocator and is often leaked. Not exception safe.
     */
    char* LookupCustomMetadataMap(const char* key, OrtAllocator* allocator)
     ...
     *  The OrtAllocator instances must be valid at the point of memory release.
     */
    AllocatedStringPtr LookupCustomMetadataMapAllocated(const char* key, OrtAllocator* allocator)
        const;  ///< Wraps OrtApi::ModelMetadataLookupCustomMetadataMap
...
struct Session : Base<OrtSession> {
    ...
    Session(Env& env, const ORTCHAR_T* model_path,
            const SessionOptions& options);  ///< Wraps OrtApi::CreateSession
    Session(
        Env& env, const ORTCHAR_T* model_path, const SessionOptions& options,
        OrtPrepackedWeightsContainer*
            prepacked_weights_container);  ///< Wraps
                                           ///< OrtApi::CreateSessionWithPrepackedWeightsContainer
    Session(Env& env, const void* model_data, size_t model_data_length,
            const SessionOptions& options);  ///< Wraps OrtApi::CreateSessionFromArray
     ...
     * model. Common errors are.. (TODO)
     *
     * \param[in] run_options
     * \param[in] input_names Array of null terminated strings of length input_count that is the
     * list of input names \param[in] input_values Array of Value objects of length input_count that
     * is the list of input values \param[in] input_count Number of inputs (the size of the
     * input_names & input_values arrays) \param[in] output_names Array of C style strings of length
     * output_count that is the list of output names \param[in] output_count Number of outputs (the
     * size of the output_names array) \return A std::vector of Value objects that directly maps to
     * the output_count (eg. output_name[0] is the first entry of the returned vector)
     */
     ...
     * Same as Run(const RunOptions&, const char* const*, const Value*, size_t,const char* const*,
     * size_t)
     */
    void Run(const RunOptions& run_options, const char* const* input_names,
             const Value* input_values, size_t input_count, const char* const* output_names,
             Value* output_values, size_t output_count);

    void Run(const RunOptions& run_options,
             const struct IoBinding&);  ///< Wraps OrtApi::RunWithBinding
    ...
    size_t GetOverridableInitializerCount()
        const;  ///< Returns the number of inputs that have defaults that can be overridden
     ...
     * by the specified allocator and is often leaked. Not exception safe.
     */
    char* GetInputName(size_t index,
                       OrtAllocator* allocator) const;  ///< Wraps OrtApi::SessionGetInputName
     ...
     * by the specified allocator and is often leaked. Not exception safe.
     */
    char* GetOutputName(size_t index,
                        OrtAllocator* allocator) const;  ///< Wraps OrtApi::SessionGetOutputName
     ...
     * by the specified allocator and is often leaked. Not exception safe.
     */
    char* GetOverridableInitializerName(size_t index, OrtAllocator* allocator)
     ...
     *  The OrtAllocator instances must be valid at the point of memory release.
     */
    AllocatedStringPtr GetOverridableInitializerNameAllocated(size_t index, OrtAllocator* allocator)
        const;  ///< Wraps OrtApi::SessionGetOverridableInitializerName
    ...
    AllocatedStringPtr EndProfilingAllocated(
        OrtAllocator* allocator) const;        ///< Wraps OrtApi::SessionEndProfiling
    ...
    TypeInfo GetOverridableInitializerTypeInfo(
        size_t index) const;  ///< Wraps OrtApi::SessionGetOverridableInitializerTypeInfo
};
...
struct TensorTypeAndShapeInfo : Base<OrtTensorTypeAndShapeInfo> {
    ...
    explicit TensorTypeAndShapeInfo(OrtTensorTypeAndShapeInfo* p)
        : Base<OrtTensorTypeAndShapeInfo>{p} {}  ///< Used for interop with the C API
    ...
    void GetDimensions(int64_t* values,
                       size_t values_count) const;  ///< Wraps OrtApi::GetDimensions
    void GetSymbolicDimensions(const char** values,
                               size_t values_count) const;  ///< Wraps OrtApi::GetSymbolicDimensions

    std::vector<int64_t> GetShape()
        const;  ///< Uses GetDimensionsCount & GetDimensions to return a std::vector of the shape
};
...
struct SequenceTypeInfo : Base<OrtSequenceTypeInfo> {
    ...
    explicit SequenceTypeInfo(OrtSequenceTypeInfo* p)
        : Base<OrtSequenceTypeInfo>{p} {}  ///< Used for interop with the C API
...
struct MapTypeInfo : Base<OrtMapTypeInfo> {
    ...
    explicit MapTypeInfo(OrtMapTypeInfo* p)
        : Base<OrtMapTypeInfo>{p} {}  ///< Used for interop with the C API
...
struct TypeInfo : Base<OrtTypeInfo> {
    ...
    Unowned<TensorTypeAndShapeInfo> GetTensorTypeAndShapeInfo()
        const;  ///< Wraps OrtApi::CastTypeInfoToTensorInfo
    Unowned<SequenceTypeInfo> GetSequenceTypeInfo()
        const;                                    ///< Wraps OrtApi::CastTypeInfoToSequenceTypeInfo
...
struct Value : Base<OrtValue> {
     ...
     * OrtApi::CreateTensorWithDataAsOrtValue. \tparam T The numeric datatype. This API is not
     * suitable for strings. \param info Memory description of where the p_data buffer resides (CPU
     * vs GPU etc). \param p_data Pointer to the data buffer. \param p_data_element_count The number
     * of elements in the data buffer. \param shape Pointer to the tensor shape dimensions. \param
     ...
     * buffer resides (CPU vs GPU etc). \param p_data Pointer to the data buffer. \param
     * p_data_byte_count The number of bytes in the data buffer. \param shape Pointer to the tensor
     * shape dimensions. \param shape_len The number of tensor shape dimensions. \param type The
     * data type.
     */
    ...
    /// <param name="p_data">pointer to the user supplied buffer, use nullptr for fully sparse
    /// tensors</param> <param name="dense_shape">a would be dense shape of the tensor</param>
    /// <param name="values_shape">non zero values shape. Use a single 0 shape for fully sparse
    /// tensors.</param> <returns></returns>
    template <typename T>
    ...
    /// of the user provided data and does not modify it. The lifespan of user provided buffers
    /// should eclipse the life span of the resulting OrtValue. This call constructs an instance
    /// that only contain a pointer to non-zero values. To fully populate the sparse tensor call
    /// Use<Format>Indices() API below to supply a sparse format specific indices. This API is not
    /// suitable for string data. Use CreateSparseTensor() with allocator specified so strings can
    ...
    /// tensors</param> <param name="dense_shape">a would be dense shape of the tensor</param>
    /// <param name="values_shape">non zero values shape. Use a single 0 shape for fully sparse
    /// tensors.</param> <param name="type">data type</param> <returns>Ort::Value instance
    /// containing SparseTensor</returns>
    static Value CreateSparseTensor(const OrtMemoryInfo* info, void* p_data,
                                    const Shape& dense_shape, const Shape& values_shape,
                                    ONNXTensorElementDataType type);

    /// <summary>
    /// Supplies COO format specific indices and marks the contained sparse tensor as being a COO
    /// format tensor. Values are supplied with a CreateSparseTensor() API. The supplied indices are
    /// not copied and the user allocated buffers lifespan must eclipse that of the OrtValue. The
    /// location of the indices is assumed to be the same as specified by OrtMemoryInfo argument at
    /// the creation time.
    /// </summary>
    /// <param name="indices_data">pointer to the user allocated buffer with indices. Use nullptr
    /// for fully sparse tensors.</param> <param name="indices_num">number of indices entries. Use 0
    /// for fully sparse tensors</param>
    void UseCooIndices(int64_t* indices_data, size_t indices_num);

    /// <summary>
    /// Supplies CSR format specific indices and marks the contained sparse tensor as being a CSR
    /// format tensor. Values are supplied with a CreateSparseTensor() API. The supplied indices are
    /// not copied and the user allocated buffers lifespan must eclipse that of the OrtValue. The
    /// location of the indices is assumed to be the same as specified by OrtMemoryInfo argument at
    /// the creation time.
    /// </summary>
    /// <param name="inner_data">pointer to the user allocated buffer with inner indices or nullptr
    /// for fully sparse tensors</param> <param name="inner_num">number of csr inner indices or 0
    /// for fully sparse tensors</param> <param name="outer_data">pointer to the user allocated
    /// buffer with outer indices or nullptr for fully sparse tensors</param> <param
    /// name="outer_num">number of csr outer indices or 0 for fully sparse tensors</param>
    void UseCsrIndices(int64_t* inner_data, size_t inner_num, int64_t* outer_data,
                       size_t outer_num);

    /// <summary>
    /// Supplies BlockSparse format specific indices and marks the contained sparse tensor as being
    /// a BlockSparse format tensor. Values are supplied with a CreateSparseTensor() API. The
    /// supplied indices are not copied and the user allocated buffers lifespan must eclipse that of
    /// the OrtValue. The location of the indices is assumed to be the same as specified by
    /// OrtMemoryInfo argument at the creation time.
    /// </summary>
    /// <param name="indices_shape">indices shape or a {0} for fully sparse</param>
    /// <param name="indices_data">user allocated buffer with indices or nullptr for fully spare
    /// tensors</param>
    ...
    /// </summary>
    /// <typeparam name="T">numeric data type only. String data enum must be specified
    /// explicitly.</typeparam> <param name="allocator">allocator to use</param> <param
    /// name="dense_shape">a would be dense shape of the tensor</param>
    ...
    /// The data must be supplied by on of the FillSparseTensor<Format>() methods that take both
    /// non-zero values and indices. The data will be copied into a buffer that would be allocated
    /// using the supplied allocator. Use this API to create OrtValues that contain sparse tensors
    /// with all supported data types including strings.
    /// </summary>
    /// <param name="allocator">allocator to use. The allocator lifespan must eclipse that of the
    /// resulting OrtValue</param> <param name="dense_shape">a would be dense shape of the
    /// tensor</param> <param name="type">data type</param> <returns>an instance of
    /// Ort::Value</returns>
    static Value CreateSparseTensor(OrtAllocator* allocator, const Shape& dense_shape,
                                    ONNXTensorElementDataType type);
    ...
    /// CreateSparseTensor() API and copy the values and COO indices into it. If data_mem_info
    /// specifies that the data is located at difference device than the allocator, a X-device copy
    ...
    void FillSparseTensorCoo(const OrtMemoryInfo* data_mem_info,
                             const OrtSparseValuesParam& values_param, const int64_t* indices_data,
                             size_t indices_num);

    /// <summary>
    /// The API will allocate memory using the allocator instance supplied to the
    /// CreateSparseTensor() API and copy the values and CSR indices into it. If data_mem_info
    /// specifies that the data is located at difference device than the allocator, a X-device copy
    /// will be performed if possible.
    /// </summary>
    /// <param name="data_mem_info">specified buffer memory description</param>
    /// <param name="values">values buffer information</param>
    /// <param name="inner_indices_data">csr inner indices pointer or nullptr for fully sparse
    /// tensors</param> <param name="inner_indices_num">number of csr inner indices or 0 for fully
    /// sparse tensors</param> <param name="outer_indices_data">pointer to csr indices data or
    /// nullptr for fully sparse tensors</param> <param name="outer_indices_num">number of csr outer
    /// indices or 0</param>
    void FillSparseTensorCsr(const OrtMemoryInfo* data_mem_info, const OrtSparseValuesParam& values,
                             const int64_t* inner_indices_data, size_t inner_indices_num,
                             const int64_t* outer_indices_data, size_t outer_indices_num);

    /// <summary>
    /// The API will allocate memory using the allocator instance supplied to the
    /// CreateSparseTensor() API and copy the values and BlockSparse indices into it. If
    /// data_mem_info specifies that the data is located at difference device than the allocator, a
    /// X-device copy will be performed if possible.
    /// </summary>
    /// <param name="data_mem_info">specified buffer memory description</param>
    /// <param name="values">values buffer information</param>
    /// <param name="indices_shape">indices shape. use {0} for fully sparse tensors</param>
    /// <param name="indices_data">pointer to indices data or nullptr for fully sparse
    /// tensors</param>
    void FillSparseTensorBlockSparse(const OrtMemoryInfo* data_mem_info,
                                     const OrtSparseValuesParam& values, const Shape& indices_shape,
    ...
    /// <returns>Pinter to the internal sparse tensor buffer containing indices. Do not free this
    /// pointer.</returns>
    template <typename T>
    const T* GetSparseTensorIndicesData(OrtSparseIndicesFormat indices_format,
    ...
    template <typename T>
    static Value CreateOpaque(const char* domain, const char* type_name,
                              const T&);  ///< Wraps OrtApi::CreateOpaqueValue

    template <typename T>
    void GetOpaqueData(const char* domain, const char* type_name,
                       T&) const;  ///< Wraps OrtApi::GetOpaqueValue
    ...
    bool IsTensor()
        const;  ///< Returns true if Value is a tensor, false for other types like map/sequence/etc
    bool HasValue() const;  /// < Return true if OrtValue contains data and returns false if the
                            /// OrtValue is a None
    ...
    size_t GetCount() const;  // If a non tensor, returns 2 for map and N for sequence, where N is
                              // the number of elements
    Value GetValue(int index, OrtAllocator* allocator) const;

    /// <summary>
    /// This API returns a full length of string data contained within either a tensor or a sparse
    /// Tensor. For sparse tensor it returns a full length of stored non-empty strings (values). The
    /// API is useful for allocating necessary memory and calling GetStringTensorContent().
    /// </summary>
    /// <returns>total length of UTF-8 encoded bytes contained. No zero terminators
    /// counted.</returns>
    size_t GetStringTensorDataLength() const;

    /// <summary>
    /// The API copies all of the UTF-8 encoded string data contained within a tensor or a sparse
    /// tensor into a supplied buffer. Use GetStringTensorDataLength() to find out the length of the
    /// buffer to allocate. The user must also allocate offsets buffer with the number of entries
    /// equal to that of the contained strings.
    ///
    ...
    /// <param name="offsets_count">count of offsets, must be equal to the number of strings
    /// contained.
    ///   that can be obtained from the shape of the tensor or from
    ///   GetSparseTensorValuesTypeAndShapeInfo() for sparse tensors</param>
    void GetStringTensorContent(void* buffer, size_t buffer_length, size_t* offsets,
    ...
    /// first.
    /// </summary>
    /// <typeparam name="T">numeric data types only. Use GetStringTensor*() to retrieve
    /// strings.</typeparam> <returns>a pointer to the internal values buffer. Do not free this
    /// pointer.</returns>
    template <typename T>
...
struct MemoryInfo : Base<OrtMemoryInfo> {
    ...
    explicit MemoryInfo(OrtMemoryInfo* p)
        : Base<OrtMemoryInfo>{p} {}  ///< Used for interop with the C API
...
struct ArenaCfg : Base<OrtArenaCfg> {
     ...
     * \param arena_extend_strategy -  use -1 to allow ORT to choose the default, 0 =
     * kNextPowerOfTwo, 1 = kSameAsRequested \param initial_chunk_size_bytes - use -1 to allow ORT
     * to choose the default \param max_dead_bytes_per_chunk - use -1 to allow ORT to choose the
     * default See docs/C_API.md for details on what the following parameters mean and how to choose
     * these values
     */
...
struct CustomOpApi {
    ...
    void SetDimensions(OrtTensorTypeAndShapeInfo* info, _In_ const int64_t* dim_values,
                       size_t dim_count);
    ...
    OrtValue* KernelContext_GetOutput(OrtKernelContext* context, _In_ size_t index,
    ...
    OrtOpAttr* CreateOpAttr(_In_ const char* name, _In_ const void* data, _In_ int len,
                            _In_ OrtOpAttrType type);

    void ReleaseOpAttr(_Frees_ptr_opt_ OrtOpAttr* op_attr);

    OrtOp* CreateOp(_In_ const OrtKernelInfo* info, _In_ const char* op_name,
                    ...
                    _In_ int input_count, _In_ int output_count);

    void InvokeOp(_In_ const OrtKernelContext* context, _In_ const OrtOp* ort_op,
...
struct CustomOpBase : OrtCustomOp {
    CustomOpBase() {
        ...
        OrtCustomOp::CreateKernel = [](const OrtCustomOp* this_, const OrtApi* api,
...
#pragma warning(pop)
    ...
    // Default implementation of GetExecutionProviderType that returns nullptr to default to the CPU
    // provider
