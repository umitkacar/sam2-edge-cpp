...
public:
    /**
      ...
      @param tp_options optional set of parameters controlling the number of intra and inter op
      threads for the global threadpools.
      @param create_global_thread_pools determine if this function will create the global
      threadpools or not.
    */
    ...
    logging::LoggingManager* GetLoggingManager() const { return logging_manager_.get(); }
    ...
    bool EnvCreatedWithGlobalThreadPools() const { return create_global_thread_pools_; }
     ...
     * Return an error if an allocator with the same OrtMemoryInfo is already registered.
     */
    Status CreateAndRegisterAllocator(const OrtMemoryInfo& mem_info,
    ...
    /**
     * Removes registered allocator that was previously registered for sharing between multiple
     * sessions.
     */
