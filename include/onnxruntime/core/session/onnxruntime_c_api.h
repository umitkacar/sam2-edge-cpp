 ...
 *
 * It is a set of header only wrapper classes around the C API. The goal is to turn the C style
 * return value error codes into C++ exceptions, and to automate memory management through standard
 * C++ RAII principles.
 *
 * \addtogroup Global
 * ONNX Runtime C API
...
#define ORT_API_STATUS(NAME, ...)                                                         \
    _Success_(return == 0) _Check_return_ _Ret_maybenull_ OrtStatusPtr ORT_API_CALL NAME( \
        __VA_ARGS__)                                                                      \
...
// Used in *.cc files. Almost as same as ORT_API_STATUS, except without ORT_MUST_USE_RESULT and
// ORT_EXPORT
...
typedef enum ONNXTensorElementDataType {
    ...
    ONNX_TENSOR_ELEMENT_DATA_TYPE_BFLOAT16     // Non-IEEE floating-point format based on IEEE754
                                               // single-precision
 ...
 *
 * In typical API usage, specifying a logging severity level specifies the minimum severity of log
 * messages to show.
 */
...
ORT_RUNTIME_CLASS(
    Session);  // Don't call ReleaseSession from Dllmain (because session owns a thread pool)
 ...
 *
 * Structure of function pointers that defines a memory allocator. This can be created and filled in
 * by the user for custom allocators.
 *
 * When an allocator is passed to any function, be sure that the allocator object is not destroyed
 * until the last allocated object using it is freed.
 */
typedef struct OrtAllocator {
    uint32_t version;  ///< Must be initialized to ORT_API_VERSION
    void*(ORT_API_CALL* Alloc)(
        struct OrtAllocator* this_,
        size_t size);  ///< Returns a pointer to an allocated block of `size` bytes
    void(ORT_API_CALL* Free)(
        struct OrtAllocator* this_,
        void* p);  ///< Free a block of memory previously allocated with OrtAllocator::Alloc
    const struct OrtMemoryInfo*(ORT_API_CALL* Info)(
        const struct OrtAllocator*
            this_);  ///< Return a pointer to an ::OrtMemoryInfo that describes this allocator
} OrtAllocator;

typedef void(ORT_API_CALL* OrtLoggingFunction)(void* param, OrtLoggingLevel severity,
...
/** \brief Memory types for allocated memory, execution provider specific types should be extended
 * in each provider.
 */
// Whenever this struct is updated, please also update the MakeKey function in onnxruntime / core /
// framework / execution_provider.cc
typedef enum OrtMemType {
    OrtMemTypeCPUInput = -2,  ///< Any CPU memory used by non-CPU execution provider
    OrtMemTypeCPUOutput =
        -1,  ///< CPU accessible memory outputted by non-CPU execution provider, i.e. CUDA_PINNED
    OrtMemTypeCPU = OrtMemTypeCPUOutput,  ///< Temporary CPU accessible memory allocated by non-CPU
...
typedef struct OrtCUDAProviderOptions {
    ...
    /** \brief Flag indicating if copying needs to take place on the same stream as the compute
     * stream in the CUDA EP 0 = Use separate streams for copying and compute. 1 = Use the same
     * stream for copying and compute. Defaults to 1. WARNING: Setting this to 0 may result in data
     * races for some models. Please see issue #4829 for more details.
     */
...
typedef struct OrtROCMProviderOptions {
    ...
    /** \brief Flag indicating if copying needs to take place on the same stream as the compute
     * stream in the ROCM EP 0 = Use separate streams for copying and compute. 1 = Use the same
     * stream for copying and compute. Defaults to 1. WARNING: Setting this to 0 may result in data
     * races for some models. Please see issue #4829 for more details.
     */
    int do_copy_in_default_stream;
...
typedef struct OrtTensorRTProviderOptions {
    ...
    int trt_int8_use_native_calibration_table;  // use native TensorRT generated calibration table.
                                                // Default 0 = false, nonzero = true
    int trt_dla_enable;                         // enable DLA. Default 0 = false, nonzero = true
    int trt_dla_core;                           // DLA core number. Default 0
    int trt_dump_subgraphs;             // dump TRT subgraph. Default 0 = false, nonzero = true
    int trt_engine_cache_enable;        // enable engine caching. Default 0 = false, nonzero = true
    const char* trt_engine_cache_path;  // specify engine cache path
    int trt_engine_decryption_enable;   // enable engine decryption. Default 0 = false, nonzero =
                                        // true
    const char* trt_engine_decryption_lib_path;  // specify engine decryption library path
    int trt_force_sequential_engine_build;  // force building TensorRT engine sequentially. Default
                                            // 0 = false, nonzero = true
    // This is the legacy struct and don't add new fields here.
    // For new field that can be represented by string, please add it in
    // include/onnxruntime/core/providers/tensorrt/tensorrt_provider_options.h For non-string field,
    // need to create a new separate api to handle it.
...
typedef struct OrtOpenVINOProviderOptions {
     ...
     *
     * Valid settings are one of: "CPU_FP32", "GPU_FP32", "GPU_FP16", "MYRIAD_FP16", "VAD-M_FP16" or
     * "VAD-F_FP32"
     */
...
struct OrtApiBase {
     ...
     * \return The ::OrtApi for the version requested, nullptr will be returned if this version is
     * unsupported, for example when using a runtime older than the version created with this header
     * file.
     */
    const OrtApi*(ORT_API_CALL* GetApi)(uint32_t version)NO_EXCEPTION;
    const char*(ORT_API_CALL* GetVersionString)(void)
        NO_EXCEPTION;  ///< Returns a null terminated string of the version of the Onnxruntime
                       ///< library (eg: "1.8.1")
};
 ...
 * Argument is an onnxruntime built-in type which will be provided when thread pool calls
 ...
 * Onnxruntime will throw exception on return value of nullptr or 0, indicating that the function
...
struct OrtApi {
     ...
     * \param[in] logger_param A pointer to arbitrary data passed as the ::OrtLoggingFunction
     * `param` parameter to `logging_function`. \param[in] log_severity_level The log severity
     * level. \param[in] logid The log identifier. \param[out] out Returned newly created OrtEnv.
     * Must be freed with OrtApi::ReleaseEnv
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(CreateEnvWithCustomLogger, OrtLoggingFunction logging_function,
                    _In_opt_ void* logger_param, OrtLoggingLevel log_severity_level,
     ...
     *
     * Will not return until the model run has completed. Multiple threads might be used to run the
     * model based on the options in the ::OrtSession and settings used when creating the ::OrtEnv
     *
     * \param[in] session
     ...
     *     an array of nullptr values, in this case ::OrtValue objects will be allocated and
     * pointers to them will be set into the `outputs` array.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * one of these functions to enable them in the session:<br>
     ...
     * will give you better performance. See [docs/ONNX_Runtime_Perf_Tuning.md] for more details.
     *
     * \param[in] options
     ...
     * and generate a memory pattern for future request. So next time we could just do one
     * allocation with a big chunk for all the internal memory allocation. \note Memory pattern
     * optimization is only available when Sequential Execution mode is enabled (see
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SetSessionLogVerbosityLevel, _Inout_ OrtSessionOptions* options,
                    int session_log_verbosity_level);

    /** \brief Set session log severity level
     *
     * \param[in] options
     * \param[in] session_log_severity_level The log severity level (refer to ::OrtLoggingLevel for
     * possible values).
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SetSessionLogSeverityLevel, _Inout_ OrtSessionOptions* options,
                    int session_log_severity_level);

    /** \brief Set the optimization level to apply when loading a graph
     *
     * Please see https://www.onnxruntime.ai/docs/resources/graph-optimizations.html for an in-depth
     * explanation \param[in,out] options The session options object \param[in]
     * graph_optimization_level The optimization level
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     *
     * When running a single node operation, ex. add, this sets the maximum number of threads to
     * use.
     *
     ...
     *
     * If nodes can be run in parallel, this sets the maximum number of threads to use to run them
     * in parallel.
     *
     * \note If sequential execution is enabled this value is ignored, it acts as if it was set
     * to 1.
     *
     * \param[in] options
     ...
     *
     * \note The OrtCustomOp* pointer must remain valid until the ::OrtCustomOpDomain using it is
     * released
     *
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(CustomOpDomain_Add, _Inout_ OrtCustomOpDomain* custom_op_domain,
     ...
     *
     * Loads a shared library (dll on windows, so on linux, etc) named 'library_path' and looks for
     *this entry point: OrtStatus* RegisterCustomOps(OrtSessionOptions * options, const OrtApiBase*
     *api); It then passes in the provided session options to this function along with the api base.
     * The handle to the loaded library is returned in library_handle. It can be freed by the caller
     *after all sessions using the passed in session options are destroyed, or if an error occurs
     *and it is non null.
     *
     * \param[in] options
     * \param[in] library_path
     * \param[out] library_handle OS specific handle to the loaded library (Use FreeLibrary on
     *Windows, dlclose on Linux, etc.. to unload)
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(RegisterCustomOpsLibrary, _Inout_ OrtSessionOptions* options,
                    _In_ const char* library_path, _Outptr_ void** library_handle);
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionGetOverridableInitializerCount, _In_ const OrtSession* session,
                    _Out_ size_t* out);

    /** \brief Get input type information
     *
     * \param[in] session
     * \param[in] index Must be between 0 (inclusive) and what OrtApi::SessionGetInputCount returns
     * (exclusive) \param[out] type_info Must be freed with OrtApi::ReleaseTypeInfo
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     *
     * \param[in] session
     * \param[in] index Must be between 0 (inclusive) and what OrtApi::SessionGetOutputCount returns
     * (exclusive) \param[out] type_info Must be freed with OrtApi::ReleaseTypeInfo
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * OrtApi::SessionGetOverridableInitializerCount returns (exclusive) \param[out] type_info Must
     * be freed with OrtApi::ReleaseTypeInfo
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionGetOverridableInitializerTypeInfo, _In_ const OrtSession* session,
     ...
     * \param[in] session
     * \param[in] index Must be between 0 (inclusive) and what OrtApi::SessionGetInputCount returns
     * (exclusive) \param[in] allocator \param[out] value Set to a null terminated UTF-8 encoded
     * string allocated using `allocator`. Must be freed using `allocator`.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionGetInputName, _In_ const OrtSession* session, size_t index,
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionGetOutputName, _In_ const OrtSession* session, size_t index,
                    _Inout_ OrtAllocator* allocator, _Outptr_ char** value);
     ...
     * \param[in] session
     * \param[in] index Must be between 0 (inclusive) and what
     * OrtApi::SessionGetOverridableInitializerCount returns (exclusive) \param[in] allocator
     * \param[out] value Set to a null terminated UTF-8 encoded string allocated using `allocator`.
     * Must be freed using `allocator`.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionGetOverridableInitializerName, _In_ const OrtSession* session,
                    size_t index, _Inout_ OrtAllocator* allocator, _Outptr_ char** value);
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(RunOptionsSetRunLogVerbosityLevel, _Inout_ OrtRunOptions* options,
     ...
     *
     * \param[in] options
     * \param[in] log_severity_level The log severity level (refer to ::OrtLoggingLevel for possible
     * values).
     */
    ORT_API2_STATUS(RunOptionsSetRunLogSeverityLevel, _Inout_ OrtRunOptions* options,
     ...
     *
     * \param[in] options
     * \param[out] log_severity_level The log severity level (refer to ::OrtLoggingLevel for
     * possible values).
     */
    ORT_API2_STATUS(RunOptionsGetRunLogSeverityLevel, _In_ const OrtRunOptions* options,
     ...
     *                     Do not free this value, it is owned by `options`. It will be invalidated
     * if the run tag changes (i.e., with OrtApi::RunOptionsSetRunTag) or `options` is freed.
     */
     ...
     *
     * If a currently executing session needs to be force terminated, this can be called from
     * another thread to force it to fail with an error.
     *
     * \param[in] options
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     ...
     *
     * Used so the OrtRunOptions instance can be used in a new OrtApi::Run call without it instantly
     * terminating
     *
     * \param[in] options
     *
     ...
     *
     * Create a tensor with user's buffer. You can fill the buffer either before calling this
     * function or after. p_data is owned by caller. ReleaseValue won't release p_data.
     *
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(CreateTensorWithDataAsOrtValue, _In_ const OrtMemoryInfo* info,
                    _Inout_ void* p_data, size_t p_data_len, _In_ const int64_t* shape,
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(FillStringTensor, _Inout_ OrtValue* value, _In_ const char* const* s,
                    size_t s_len);
     ...
     *
     * \param[in] value A tensor of type ONNX_TENSOR_ELEMENT_DATA_TYPE_STRING
     * \param[in] s Buffer to sequentially write all tensor strings to. Each string is NOT
     * null-terminated. \param[in] s_len Number of bytes of buffer pointed to by \p s (Get it from
     * OrtApi::GetStringTensorDataLength) \param[out] offsets Array of start offsets into the
     * strings written to \p s \param[in] offsets_len Number of elements in offsets
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(GetStringTensorContent, _In_ const OrtValue* value,
                    _Out_writes_bytes_all_(s_len) void* s, size_t s_len,
                    _Out_writes_all_(offsets_len) size_t* offsets, size_t offsets_len);
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SetDimensions, OrtTensorTypeAndShapeInfo* info, _In_ const int64_t* dim_values,
                    size_t dim_count);
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(GetDimensionsCount, _In_ const OrtTensorTypeAndShapeInfo* info,
                    _Out_ size_t* out);

    /** \brief Get dimensions in ::OrtTensorTypeAndShapeInfo
     *
     * \param[in] info
     * \param[out] dim_values Array with `dim_values_length` elements. On return, filled with the
     * dimensions stored in the ::OrtTensorTypeAndShapeInfo \param[in] dim_values_length Number of
     * elements in `dim_values`. Use OrtApi::GetDimensionsCount to get this value
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     *
     * \param[in] info
     * \param[in] dim_params Array with `dim_params_length` elements. On return filled with pointers
     * to null terminated strings of the dimension names \param[in] dim_params_length Number of
     * elements in `dim_params`. Use OrtApi::GetDimensionsCount to get this value
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     *
     * Return the number of elements specified by the tensor shape (all dimensions multiplied by
     * each other). For 0 dimensions, 1 is returned. If any dimension is less than 0, the result is
     * always -1.
     *
     * Examples:<br>
     * [] = 1<br>
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(GetTensorShapeElementCount, _In_ const OrtTensorTypeAndShapeInfo* info,
                    _Out_ size_t* out);
     ...
     *
     * Special case version of OrtApi::CreateMemoryInfo for CPU based memory. Same as using
     * OrtApi::CreateMemoryInfo with name = "Cpu" and id = 0.
     *
     * \param[in] type
     * \param[in] mem_type
     * \param[out] out
     ...
     *
     * \param[in] ptr
     * \param[out] out Writes null terminated string to this pointer. Do NOT free the returned
     * pointer. It is valid for the lifetime of the ::OrtMemoryInfo
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ...
    ORT_API2_STATUS(AllocatorAlloc, _Inout_ OrtAllocator* ort_allocator, size_t size,
                    _Outptr_ void** out);
     ...
     *
     * The default allocator is a CPU based, non-arena. Always returns the same pointer to the same
     * default allocator.
     *
     ...
     *
     * Override symbolic dimensions (by specific denotation strings) with actual values if known at
     * session initialization time to enable optimizations that can take advantage of fixed values
     * (such as memory planning, etc)
     *
     * \param[in] options
     ...
     * \param[out] out Created ::OrtValue that holds the element requested. Must be freed with
     * OrtApi::ReleaseValue
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     *
     * To construct a map (ONNX_TYPE_MAP), use num_values = 2 and `in` should be an array of 2
     * ::OrtValue%s representing keys and values.<br>
     *
     * To construct a sequence (ONNX_TYPE_SEQUENCE), use num_values = N where N is the number of the
     * elements in the sequence. 'in' should be an array of N ::OrtValue%s.
     *
    ...
    ORT_API2_STATUS(CreateValue, _In_reads_(num_values) const OrtValue* const* in,
                    size_t num_values, enum ONNXType value_type, _Outptr_ OrtValue** out);
     ...
     * the type of which must be known to both ORT and the client program. That data container may
     * or may not match the internal representation of the Opaque type. The sizeof(data_container)
     * is passed for verification purposes.
     *
    ...
    ORT_API2_STATUS(CreateOpaqueValue, _In_z_ const char* domain_name, _In_z_ const char* type_name,
                    _In_ const void* data_container, size_t data_container_size,
                    _Outptr_ OrtValue** out);
     ...
     * \param[out] data_container_size Size in bytes of the buffer pointed to by data_container.
     * Must match the size of the internal buffer.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(GetOpaqueValue, _In_ const char* domain_name, _In_ const char* type_name,
                    _In_ const OrtValue* in, _Out_ void* data_container,
     ...
     * \see ::OrtCustomOp
     */
    ORT_API2_STATUS(KernelContext_GetInputCount, _In_ const OrtKernelContext* context,
                    _Out_ size_t* out);
     ...
     *
     * \param[in] type_info
     * \param[out] denotation Pointer to the null terminated denotation string is written to this
     * pointer. This pointer is valid until the object is destroyed or the name is changed, do not
     * free. \param[out] len Length in bytes of the string returned in `denotation`
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     *
     * This api augments ::OrtTypeInfo to return an ::OrtSequenceTypeInfo when the type is a
     * sequence. The ::OrtSequenceTypeInfo has additional information about the sequence's element
     * type.
     *
     * This is used by WinML to support model reflection APIs.
     *
     ...
     * \param[in] session
     * \param[in] allocator
     * \param[out] out Null terminated string of the filename, allocated using `allocator`. Must be
     * freed using `allocator`
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * \param[in] allocator
     * \param[out] value Set to a null terminated string allocated using `allocator`. Must be freed
     * using `allocator`
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * \param[out] value Set to a null terminated string allocated using `allocator`. Must be freed
     * using `allocator` `value` will be set to nullptr if the given key is not found in the custom
     * metadata map.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(ModelMetadataLookupCustomMetadataMap,
                    _In_ const OrtModelMetadata* model_metadata, _Inout_ OrtAllocator* allocator,
                    _In_ const char* key, _Outptr_result_maybenull_ char** value);
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(ModelMetadataGetVersion, _In_ const OrtModelMetadata* model_metadata,
                    _Out_ int64_t* value);
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(CreateEnvWithGlobalThreadPools, OrtLoggingLevel log_severity_level,
                    _In_ const char* logid, _In_ const OrtThreadingOptions* tp_options,
                    _Outptr_ OrtEnv** out);
     ...
     * OrtApi::ReleaseThreadingOptions \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * \param[in] model_metadata
     * \param[in] allocator
     * \param[out] keys Array of null terminated strings (array count = num_keys) allocated using
     * `allocator`. The strings and the pointer array must be freed using `allocator` `keys` will be
     * set to nullptr if the custom metadata map is empty. \param[out] num_keys Set to the number of
     * elements in the `keys` array
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     *
     */
    ORT_API2_STATUS(AddFreeDimensionOverrideByName, _Inout_ OrtSessionOptions* options,
     ...
     *
     * \note The providers in the list are not guaranteed to be usable. They may fail to load due to
     * missing system dependencies. For example, if the CUDA/cuDNN libraries are not installed, the
     * CUDA provider will report an error when it is added to the session options.
     *
     * \param[out] out_ptr Set to a pointer to an array of null terminated strings of the available
     * providers. The entries and the array itself must be freed using
     * OrtApi::ReleaseAvailableProviders \param[out] provider_length Set to the number of entries in
     * the `out_ptr` array
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(GetStringTensorElementLength, _In_ const OrtValue* value, size_t index,
                    _Out_ size_t* out);
     ...
     * OrtApi::GetStringTensorElementLength. \param[in] index Index of the string in the tensor
     * \param[out] s The string element contents in UTF-8 encoding. The string is NOT
     * null-terminated.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(FillStringTensorElement, _Inout_ OrtValue* value, _In_ const char* s,
                    size_t index);
     ...
     *
     * If a configuration with same key exists, this will overwrite the configuration with the given
     * config_value.
     *
    ...
    ORT_API2_STATUS(RunWithBinding, _Inout_ OrtSession* session,
                    _In_ const OrtRunOptions* run_options, _In_ const OrtIoBinding* binding_ptr);
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(BindInput, _Inout_ OrtIoBinding* binding_ptr, _In_ const char* name,
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(BindOutput, _Inout_ OrtIoBinding* binding_ptr, _In_ const char* name,
                    _In_ const OrtValue* val_ptr);

    /** \brief Bind an ::OrtIoBinding output to a device
     *
     * Binds the ::OrtValue to a device which is specified by ::OrtMemoryInfo.
     * You can either create an instance of ::OrtMemoryInfo with a device id or obtain one from the
     * allocator that you have created/are using This is useful when one or more outputs have
     * dynamic shapes and, it is hard to pre-allocate and bind a chunk of memory within ::OrtValue
     * ahead of time.
     *
     ...
     *
     * Returns the names of the outputs in the order they were bound. This is useful after running
     * the model with bound outputs because the returned names are in order in which output
     * ::OrtValue are returned. This is useful if the order of outputs and their names is not known.
     *
     * \param[in] binding_ptr
     * \param[in] allocator Allocator used to allocate continuous buffers for output strings and
     * lengths. \param[out] buffer Returns an array of non-null terminated UTF-8 strings. The number
     * of strings stored is returned in the count parameter. This buffer is allocated using
     * `allocator` and must be freed using it. \param[out] lengths Returns an array of `count`
     * lengths of the strings returned in `buffer` This buffer is allocated using `allocator` and
     * must be freed using it. \param[out] count Number of strings returned. If `binding_ptr` has no
     * bound outputs, zero is returned, no memory allocation is performed and buffer and lengths are
     * set to nullptr.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(GetBoundOutputNames, _In_ const OrtIoBinding* binding_ptr,
                    ...
                    _Out_writes_all_(count) size_t** lengths, _Out_ size_t* count);

    /** \brief Get the output ::OrtValue objects from an ::OrtIoBinding
     *
     * Returns an array of pointers to individually allocated ::OrtValue%s that contain results of a
     * model execution with OrtApi::RunWithBinding The array contains the same number of
     * ::OrtValue%s and they are in the same order as they were bound with OrtApi::BindOutput or
     * OrtApi::BindOutputToDevice.
     *
     * The returned ::OrtValue%s must be released using OrtApi::ReleaseValue after they are no
     * longer needed. The array is allocated using the specified instance of the allocator and must
     * be freed using the same allocator after all the ::OrtValue%s contained therein are
     * individually released.
     *
     * \param[in] binding_ptr
     * \param[in] allocator Allocator used to allocate output array
     * \param[out] output Set to the allocated array of allocated ::OrtValue outputs. Set to nullptr
     * if there are 0 outputs. \param[out] output_count Set to number of ::OrtValue%s returned
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(GetBoundOutputValues, _In_ const OrtIoBinding* binding_ptr,
                    _In_ OrtAllocator* allocator, _Out_writes_all_(output_count) OrtValue*** output,
     ...
     *
     * For example, given a tensor with shape of [3,224,224], a pointer to the element at location
     * [2,150,128] can be retrieved
     *
     * This function only works for numeric type tensors (No strings, etc).
     * This is a no-copy method whose returned pointer is valid until the passed in ::OrtValue is
     * free'd.
     *
     * \param[in] value
     * \param[in] location_values Pointer to an array of index values that specify an element's
     * location relative to its shape \param[in] location_values_count Number of elements in
     * location_values. Must match the number of elements in the tensor's shape. \param[out] out Set
     * to a pointer to the element specified
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(TensorAt, _Inout_ OrtValue* value, const int64_t* location_values,
                    size_t location_values_count, _Outptr_ void** out);
     ...
     *
     * The default is ORT_PROJECTION_C, which means it will classify the language not in the list to
     * C also.
     *
     * \param[in] ort_env
     * \param[in] projection
     ...
     *
     * \note The timer precision varies per platform. On Windows and MacOS, the precision will be
     * ~100ns
     *
     * \param[in] session
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SessionGetProfilingStartTimeNs, _In_ const OrtSession* session,
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SetGlobalIntraOpNumThreads, _Inout_ OrtThreadingOptions* tp_options,
                    int intra_op_num_threads);
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(SetGlobalInterOpNumThreads, _Inout_ OrtThreadingOptions* tp_options,
                    int inter_op_num_threads);

    /** \brief Set global spin control options
     *
     * This will configure the global thread pool options to be used in the call to
     * OrtApi::CreateEnvWithGlobalThreadPools. Allow spinning of thread pools when their queues are
     * empty. This will set the value for both inter_op and intra_op threadpools.
     *
     * \param[in] tp_options
     ...
     * \param[in] val ::OrtValue containing the initializer. Its lifetime and the underlying
     * initializer buffer must be managed by the user (created using the
     * OrtApi::CreateTensorWithDataAsOrtValue) and it must outlive the session object to which it is
     * added.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ...
    /**
     * Create a custom environment with global threadpools and logger that will be shared across
     * sessions. Use this in conjunction with OrtApi::DisablePerSessionThreads or else the session
     * will use its own thread pools.
     *
     * \param[in] logging_function A pointer to a logging function.
     * \param[in] logger_param A pointer to arbitrary data passed as the ::OrtLoggingFunction
     * `param` parameter to `logging_function`. \param[in] log_severity_level The log severity
     * level. \param[in] logid The log identifier. \param[in] tp_options \param[out] out Newly
     * created OrtEnv. Must be freed with OrtApi::ReleaseEnv
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(CreateEnvWithCustomLoggerAndGlobalThreadPools,
                    OrtLoggingFunction logging_function, _In_opt_ void* logger_param,
     ...
     *
     * If CUDA is not available (due to a non CUDA enabled build, or if CUDA is not installed on the
     ...
     *
     * If ROCM is not available (due to a non ROCM enabled build, or if ROCM is not installed on the
     * system), this function will return failure.
     *
     * \param[in] options
     ...
     *
     * If OpenVINO is not available (due to a non OpenVINO enabled build, or if OpenVINO is not
     * installed on the system), this function will fail.
     *
     * \param[in] options
     ...
     * OrtApi::CreateEnvWithGlobalThreadPools. Flush-to-zero and denormal-as-zero are applied to
     * threads in both intra and inter global thread pool. \note This option is not needed if the
     * models used have no denormals. Having no denormals is recommended as this option may hurt
     * model accuracy.
     *
     * \param[in] tp_options
     *
     ...
     *
     * This will create the configuration of an arena that can eventually be used to define an arena
     * based allocator's behavior
     *
     * \param[in] max_mem Use 0 to allow ORT to choose the default
     * \param[in] arena_extend_strategy Use -1 to allow ORT to choose the default, 0 =
     * kNextPowerOfTwo, 1 = kSameAsRequested \param[in] initial_chunk_size_bytes Use -1 to allow ORT
     * to choose the default \param[in] max_dead_bytes_per_chunk Use -1 to allow ORT to choose the
     * default \param[in] out A pointer to an OrtArenaCfg instance
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(CreateArenaCfg, _In_ size_t max_mem, int arena_extend_strategy,
     ...
     * \param[out] value Set to a null terminated string allocated using `allocator`.  The caller is
     * responsible for freeing it using `allocator`
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     *
     * If TensorRT is not available (due to a non TensorRT enabled build, or if TensorRT is not
     * installed on the system), this function will return failure.
     *
     * \param[in] options
     ...
     *
     * Set the current device id of the GPU execution provider (CUDA/tensorrt/rocm). The device id
     * should be less than the total number of devices available. This is only useful when
     * multiple-GPUs are installed and it is required to restrict execution to a single GPU.
     *
     * \param[in] device_id
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(KernelInfoGetAttributeArray_float, _In_ const OrtKernelInfo* info,
                    _In_ const char* name, _Out_ float* out, _Inout_ size_t* size);
     ...
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(KernelInfoGetAttributeArray_int64, _In_ const OrtKernelInfo* info,
                    _In_ const char* name, _Out_ int64_t* out, _Inout_ size_t* size);
     ...
     *
     * Create the configuration of an arena that can eventually be used to define an arena based
     * allocator's behavior.
     *
     * Supported keys are (See https://onnxruntime.ai/docs/reference/api/c-api.html for details on
     * what the following parameters mean and how to choose these values.): "max_mem": Maximum
     * memory that can be allocated by the arena based allocator. Use 0 for ORT to pick the best
     * value. Default is 0. "arena_extend_strategy": 0 = kNextPowerOfTwo, 1 = kSameAsRequested. Use
     * -1 to allow ORT to choose the default. "initial_chunk_size_bytes": (Possible) Size of the
     * first allocation in the arena. Only relevant if arena strategy is `kNextPowerOfTwo`. Use -1
     * to allow ORT to choose the default. Ultimately, the first allocation size is determined by
     * the allocation memory request. "max_dead_bytes_per_chunk": Threshold of unused memory in an
     * allocated chunk of arena memory after crossing which the current chunk is chunked into 2.
     * "initial_growth_chunk_size_bytes": (Possible) Size of the second allocation in the arena.
     *  Only relevant if arena strategy is `kNextPowerOfTwo`. Use -1 to allow ORT to choose the
     * default. Ultimately, the allocation size is determined by the allocation memory request.
     ...
     *
     * This container will hold pre-packed buffers of shared initializers for sharing between
     * sessions (i.e.) if there are shared initializers that can be shared between sessions, the
     * pre-packed buffers of these (if any) may possibly be shared to provide memory footprint
     * savings. Pass this container to sessions that you would like to share pre-packed buffers of
     * shared initializers at session creation time.
     *
     ...
     *
     * Same functionality offered by OrtApi::CreateSessionFromArray except that a container that
     * contains pre-packed weights' buffers is written into/read from by the created session. This
     * is useful when used in conjunction with OrtApi::AddInitializer which injects shared
     * initializer info into sessions. Wherever possible, the pre-packed versions of these shared
     * initializers are cached in this container so that multiple sessions can just re-use these
     ...
     *
     * If TensorRT is not available (due to a non TensorRT enabled build), this function will return
     * failure.
     *
     * This is slightly different from OrtApi::SessionOptionsAppendExecutionProvider_TensorRT, it
     * takes an
     * ::OrtTensorRTProviderOptions which is publicly defined. This takes an opaque
     * ::OrtTensorRTProviderOptionsV2 which must be created with
     ...
     * to know the available keys and values. Key should be in null terminated string format of the
     * member of ::OrtTensorRTProviderOptionsV2 and value should be its related range.
     *
     * For example, key="trt_max_workspace_size" and value="2147483648"
     *
     * \param[in] tensorrt_options
     * \param[in] provider_options_keys Array of UTF-8 null-terminated string for provider options
     * keys \param[in] provider_options_values Array of UTF-8 null-terminated string for provider
     * options values \param[in] num_keys Number of elements in the `provider_option_keys` and
     ...
     * \param allocator - a ptr to an instance of OrtAllocator obtained with OrtApi::CreateAllocator
     * or OrtApi::GetAllocatorWithDefaultOptions the specified allocator will be used to allocate
     * continuous buffers for output strings and lengths. \param ptr - is a UTF-8 null terminated
     * string allocated using 'allocator'. The caller is responsible for using the same allocator to
     * free it.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     *
     * \note This is an exception in the naming convention of other Release* functions, as the name
     * of the method does not have the V2 suffix, but the type does
     */
     ...
     * Use ReleaseValue to destroy the sparse tensor, this will also release the buffer inside the
     * output value if any was allocated. \param[in,out] allocator allocator to use when performing
     * an allocation. Allocation will be performed by FillSparseTensor<Format>() APIs. The lifespan
     * of the allocator instance must eclipse the lifespan this sparse tensor instance as the same
     * allocator will be used to free memory. \param[in] dense_shape shape of the original dense
     * tensor \param[in] dense_shape_len number of shape dimensions being passed \param[in] type
     * must be one of TENSOR_ELEMENT_DATA_TYPE_xxxx \param[out] out Should be freed by calling
     * ReleaseValue
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * OrtApi::CreateSparseTensorAsOrtValue. This will allocate required memory and copy the
     * supplied NNZ values and COO indices into that memory allocation. Memory allocation is
     * performed using the allocator that was specified with OrtApi::CreateSparseTensorAsOrtValue.
     *
     * \param[in,out] ort_value ::OrtValue to populate with data
     * \param[in] data_mem_info serves to identify the location of the data to be copied. If the
     * allocator specified at the creation time has memory info that is not the same as mem_info
     * argument to this function a X-device copy will be performed. String data is assumed to be on
     * CPU and will only be copied into a CPU allocated buffer. \param[in] values_shape pointer to
     * values shape array \param[in] values_shape_len length of the values_shape \param[in] values
     * pointer to an array of values. For strings, pass const char**. \param[in] indices_data
     * pointer to a location of COO indices \param[in] indices_num number of COO indices
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(FillSparseTensorCoo, _Inout_ OrtValue* ort_value,
                    _In_ const OrtMemoryInfo* data_mem_info, _In_ const int64_t* values_shape,
                    size_t values_shape_len, _In_ const void* values,
                    _In_ const int64_t* indices_data, size_t indices_num);

    /**
     * This fills populates an empty tensor that was created using
     * OrtApi::CreateSparseTensorAsOrtValue. This will allocate required memory and copy the
     * supplied NNZ values and CSR indices into that memory allocation. Memory allocation is
     * performed using the allocator that was specified with OrtApi::CreateSparseTensorAsOrtValue.
     *
     * \param[in,out] ort_value ::OrtValue to populate with data
     * \param[in] data_mem_info serves to identify the location of the data to be copied. If the
     ...
     * CPU and will only be copied into a CPU allocated buffer. \param[in] values_shape pointer to
     * values shape array \param[in] values_shape_len length of the values_shape \param[in] values -
     * pointer to an array of values. For strings, pass const char**. \param[in] inner_indices_data
     * pointer to a location of CSR inner indices \param[in] inner_indices_num number of CSR inner
     * indices \param[in] outer_indices_data pointer to a location of CSR outer indices \param[in]
     * outer_indices_num number of CSR outer indices
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * This fills populates an empty tensor that was created using
     * OrtApi::CreateSparseTensorAsOrtValue. This will allocate required memory and copy the
     * supplied NNZ values and BlockSparse indices into that memory allocation. Memory allocation is
     * performed using the allocator that was specified with OrtApi::CreateSparseTensorAsOrtValue.
     *
     * \param[in,out] ort_value ::OrtValue to populate with data
     * \param[in] data_mem_info serves to identify the location of the data to be copied. If the
     * allocator specified at the creation time has memory info that is not the same as mem_info
     * argument to this function a X-device copy will be performed. String data is assumed to be on
     * CPU and will only be copied into a CPU allocated buffer. \param[in] values_shape \param[in]
     * values_shape_len \param[in] values structure with values information \param[in]
     * indices_shape_data pointer to a location of indices shape \param[in] indices_shape_len length
     * of the block sparse indices shape \param[in] indices_data pointer to a location of indices
     * data. Shape will determine the length of the indices data.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * Use OrtApi::ReleaseValue to destroy the sparse tensor. It would not release the supplied
     * values buffer. This function can not be used to map strings from the user allocated memory.
     * Strings must always be copied and have UTF-8 encoding. Therefore, use
     * OrtApi::CreateSparseTensorAsOrtValue above and then fill it with data using appropriate
     * Make*() function.
     *
     * \param[in] info memory info where sparse values reside.
     * \param[in,out] p_data pointer to a user allocated buffer with values. To create a full sparse
     * tensor with no non-zero values, pass nullptr \param[in] dense_shape shape of the original
     * dense tensor \param[in] dense_shape_len number of shape dimensions being passed \param[in]
     * values_shape shape of the values data. To create a fully sparse tensor with no non-zero
     * values, pass {0} shape. \param[in] values_shape_len number of values shape dimensions
     * \param[in] type must be one of TENSOR_ELEMENT_DATA_TYPE_xxxx
     * \param[out] out Should be freed by calling ReleaseValue
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(CreateSparseTensorWithValuesAsOrtValue, _In_ const OrtMemoryInfo* info,
                    _Inout_ void* p_data, _In_ const int64_t* dense_shape, size_t dense_shape_len,
     ...
     * OrtApi::CreateSparseTensorWithValuesAsOrtValue \param[in,out] indices_data pointer to a user
     * pre-allocated buffer or nullptr for fully sparse tensors. \param[in] indices_num  number of
     * COO indices. Should either be 0 for fully sparse tensors, be equal to the number of nnz
     * values specified to OrtApi::CreateSparseTensorWithValuesAsOrtValue for 1-D {nnz} indices or
     *  be twice as number of nnz values for a  2-D indices {nnz, 2}
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(UseCooIndices, _Inout_ OrtValue* ort_value, _Inout_ int64_t* indices_data,
                    size_t indices_num);
     ...
     *
     * \param[in,out] ort_value ::OrtValue instance constructed with
     * OrtApi::CreateSparseTensorWithValuesAsOrtValue \param[in,out] inner_data pointer to a user
     * pre-allocated buffer or nullptr for fully sparse tensors. \param[in] inner_num  number of
     * inner CSR indices. Should either be 0 for fully sparse tensors or be equal to the number of
     * nnz values specified to OrtApi::CreateSparseTensorWithValuesAsOrtValue. \param[in,out]
     * outer_data pointer to user pre-allocated buffer or nullptr for fully sparse tensors.
     * \param[in] outer_num number of CSR outer indices. Should either be 0 for fully sparse tensors
     * or equal to rows + 1 of the dense shape.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(UseCsrIndices, _Inout_ OrtValue* ort_value, _Inout_ int64_t* inner_data,
                    size_t inner_num, _Inout_ int64_t* outer_data, size_t outer_num);

    /**
     * The assigns BlockSparse format indices to the SparseTensor that was created by
     * OrtApi::CreateSparseTensorWithValuesAsOrtValue above. It also sets OrtSparseFormat to
     * ORT_SPARSE_BLOCK_SPARSE. This will not allocate any additional memory for data. The life span
     * of indices_data buffer must eclipse the lifespan of this ::OrtValue.
     *
     * \param[in,out] ort_value OrtValue instance constructed with
     * OrtApi::CreateSparseTensorWithValuesAsOrtValue \param[in] indices_shape pointer to indices
     * shape. Use {0} for fully sparse tensors \param[in] indices_shape_len length of the indices
     * shape \param[in,out] indices_data pointer to user pre-allocated buffer or nullptr for fully
     * sparse tensors.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ...
    /** \brief Returns sparse tensor format enum iff a given ort value contains an instance of
     * sparse tensor.
     *
    ...
    /** \brief Returns data type and shape of sparse tensor values (nnz) iff ::OrtValue contains a
     * SparseTensor.
     *
    ...
    /** \brief Returns numeric data for sparse tensor values (nnz). For string values use
     * GetStringTensor*().
     *
     ...
     * \param[in] indices_format One of the indices formats. It is an error to request a format that
     * the sparse tensor does not contain. \param[out] out an instance of
     * ::OrtTensorTypeAndShapeInfo. Must be freed by OrtApi::ReleaseTensorTypeAndShapeInfo
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * \param[in] ort_value ::OrtValue containing sparse tensor.
     * \param[in] indices_format One of the indices formats. It is an error to request a format that
     * the sparse tensor does not contain. \param[out] num_indices Pointer to where the number of
     * indices entries is returned \param[out] indices Returned pointer to the indices data. Do not
     * free the returned pointer as it refers to internal data owned by the ::OrtValue
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(GetSparseTensorIndices, _In_ const OrtValue* ort_value,
                    enum OrtSparseIndicesFormat indices_format, _Out_ size_t* num_indices,
    ...
    /**
     * \brief Sets out to 1 iff an optional type OrtValue has an element, 0 otherwise (OrtValue is
     None)
    ...
    /// \name OrtKernelContext
    /// @{
    /** \brief Used for custom operators, gets the GPU compute stream to use to launch the custom a
     * GPU kernel \see ::OrtCustomOp \param[in]  context OrtKernelContext instance \param[out] out
     * Returns pointer to a GPU compute stream that can be used to launch the custom GPU kernel. If
     * retrieving the GPU compute stream is not relevant (GPU not enabled in the build, kernel
     * partitioned to some other EP), then a nullptr is returned as the output param. Do not free or
     * mutate the returned pointer as it refers to internal data owned by the underlying session.
     *             Only use it for custom kernel launching.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(KernelContext_GetGPUComputeStream, _In_ const OrtKernelContext* context,
                    _Outptr_ void** out);
     ...
     * \param[out] mem_info ::OrtMemoryInfo of the tensor. Do NOT free the returned pointer. It is
     * valid for the lifetime of the ::OrtValue
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * \param[in] provider_name The name of the execution provider name. Currently only the
     * following values are supported: "DML". \param[in] version Must be ::ORT_API_VERSION.
     * \param[out] provider_api A void pointer containing a reference to the execution provider
     * versioned api structure. For example, the provider_api pointer can be cast to the OrtDmlApi*
     * when the provider_name is "DML".
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     *
     * \param[inout] tp_options
     * \param[in] ort_custom_join_thread_fn Custom thread join function, must not be nullptr when
     * global ort_custom_create_thread_fn is set
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ...
    /** \brief Synchronize bound outputs. The call may be necessary for some providers, such as
     * cuda, in case the system that allocated bound memory operated on a different stream. However,
     * the operation is provider specific and could be a no-op.
     *
     * \param[inout] binding_ptr
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     ...
     *
     * If CUDA is not available (due to a non CUDA enabled build), this function will return
     * failure.
     *
     * This is slightly different from OrtApi::SessionOptionsAppendExecutionProvider_CUDA, it takes
     * an
     * ::OrtCUDAProviderOptions which is publicly defined. This takes an opaque
     * ::OrtCUDAProviderOptionsV2 which must be created with OrtApi::CreateCUDAProviderOptions.
     *
     * For OrtApi::SessionOptionsAppendExecutionProvider_CUDA, the user needs to instantiate
     * ::OrtCUDAProviderOptions as well as allocate/release buffers for some members of
     * ::OrtCUDAProviderOptions. Here, OrtApi::CreateCUDAProviderOptions and
     * Ortapi::ReleaseCUDAProviderOptions will do the memory management for you.
     *
     * \param[in] options
     * \param[in] cuda_options
     *
     ...
     * to know the available keys and values. Key should be in null terminated string format of the
     * member of ::OrtCUDAProviderOptionsV2 and value should be its related range.
     *
     ...
     * GetAllocatorWithDefaultOptions() the specified allocator will be used to allocate continuous
     * buffers for output strings and lengths. \param ptr - is a UTF-8 null terminated string
     * allocated using 'allocator'. The caller is responsible for using the same allocator to free
     * it.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     ...
     *
     * If MIGraphX is not available (due to a non MIGraphX enabled build, or if MIGraphX is not
     * installed on the system), this function will return failure.
     *
     * \param[in] options
    ...
    /** \brief Replace initialized Tensors with external data with the data provided in
     * initializers.
     *
     * The function will find the initialized TensorProtos with external data in the graph with the
     * provided names and replace them with the provided tensors. The API verifies that the
     * TensorProto being replaced has an external data reference and has the same name, dimensions
     * and data type as its replacement. The replacement will occur before any of the optimizations
     * take place. The data will be copied into the graph since TensorProto can't refer to the user
     * provided buffers.
     *
     * Once the model has been loaded, the OrtValue(s) added to SessionOptions instance will be
     * removed from the internal SessionOptions copy to save memory, the user provided buffers can
     * then be deallocated and the SessionOptions instance that refers to them can be destroyed.
     *
     * \param[in] options
     * \param[in] initializer_names Array of null terminated UTF-8 encoded strings of the
     * initializers names. \param[in] initializers Array of ::OrtValue type \param[in]
     * initializers_num Number of elements in the initializer_names and initializers
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.12.
     ...
     * \param[out] op_attr Attribute that has been created, which must be released by
     * OrtApi::ReleaseOpAttr
     *
     * \since Version 1.12.
     */
    ORT_API2_STATUS(CreateOpAttr, _In_ const char* name, _In_ const void* data, _In_ int len,
                    _In_ OrtOpAttrType type, _Outptr_ OrtOpAttr** op_attr);
     ...
     * \since Version 1.12.
     */
    ORT_API2_STATUS(CreateOp, _In_ const OrtKernelInfo* info, _In_ const char* op_name,
                    _In_ const char* domain, _In_ int version,
                    _In_opt_ const char** type_constraint_names,
                    _In_opt_ const ONNXTensorElementDataType* type_constraint_values,
                    _In_opt_ int type_constraint_count,
                    _In_opt_ const OrtOpAttr* const* attr_values, _In_opt_ int attr_count,
                    _In_ int input_count, _In_ int output_count, _Outptr_ OrtOp** ort_op);
     ...
     * SessionOptionsAppendExecutionProvider_<provider name> function that should be used to add it.
     *
     * SNPE supported keys:
     *   "runtime": SNPE runtime engine, options: "CPU", "CPU_FLOAT32", "GPU",
     * "GPU_FLOAT32_16_HYBRID", "GPU_FLOAT16", "DSP", "DSP_FIXED8_TF", "AIP_FIXED_TF",
     * "AIP_FIXED8_TF". Mapping to SNPE Runtime_t definition: CPU, CPU_FLOAT32 =>
     * zdl::DlSystem::Runtime_t::CPU; GPU, GPU_FLOAT32_16_HYBRID => zdl::DlSystem::Runtime_t::GPU;
     ...
     *   SNPE Runtime_t refers to
     * https://developer.qualcomm.com/docs/snpe/group__c__plus__plus__apis.html "priority":
     * execution priority, options: "low", "normal". "buffer_type": ITensor or user buffers,
     * options: "ITENSOR", user buffer with different types - "TF8", "TF16", "UINT8", "FLOAT".
     *   "ITENSOR" -- default, ITensor which is float only.
     *   "TF8" -- quantized model required, "FLOAT" -- for both quantized or non-quantized model
     *   If SNPE is not available (due to a non Snpe enabled build or its dependencies not being
     * installed), this function will fail.
     *
...
/*
 * The OrtCustomOp structure defines a custom op's schema and its kernel callbacks. The callbacks
 * are filled in by the implementor of the custom op.
 */
struct OrtCustomOp {
    uint32_t version;  // Must be initialized to ORT_API_VERSION

    // This callback creates the kernel, which is a user defined parameter that is passed to the
    // Kernel* callbacks below.
 ...
 * SessionOptionsAppendExecutionProvider_CUDA above to access the latest functionality This function
 * always exists, but will only succeed if Onnxruntime was built with CUDA support and the CUDA
 * provider shared library exists
 *
 * \param device_id CUDA device id, starts from zero.
 */
ORT_API_STATUS(OrtSessionOptionsAppendExecutionProvider_CUDA, _In_ OrtSessionOptions* options,
               int device_id);
