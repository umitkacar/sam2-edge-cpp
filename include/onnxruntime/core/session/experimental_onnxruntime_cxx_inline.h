...
//
// These are the inline implementations of the C++ header APIs. They are in this separate file as to
// not clutter the main C++ file with implementation details.
namespace Ort {
namespace Experimental {

inline std::vector<Ort::Value> Session::Run(const std::vector<std::string>& input_names,
...
inline void Session::Run(const std::vector<std::string>& input_names,
                         const std::vector<Ort::Value>& input_values,
                         const std::vector<std::string>& output_names,
                         std::vector<Ort::Value>& output_values, const RunOptions& run_options) {
    ...
    for (auto it = input_names.begin(); it != input_names.end(); it++)
        input_names_[i++] = (*it).c_str();
    std::vector<const char*> output_names_(output_names_count, nullptr);
    i = 0;
    for (auto it = output_names.begin(); it != output_names.end(); it++)
        output_names_[i++] = (*it).c_str();
    Ort::Session::Run(run_options, input_names_.data(), input_values.data(), input_names_count,
                      output_names_.data(), output_values.data(), output_names_count);
}
...
inline std::vector<std::vector<int64_t>> Session::GetInputShapes() const {
    ...
    for (size_t i = 0; i < node_count; i++)
        out[i] = GetInputTypeInfo(i).GetTensorTypeAndShapeInfo().GetShape();
    return out;
}
...
inline std::vector<std::vector<int64_t>> Session::GetOutputShapes() const {
    ...
    std::vector<std::vector<int64_t>> out(node_count);
    for (size_t i = 0; i < node_count; i++)
        out[i] = GetOutputTypeInfo(i).GetTensorTypeAndShapeInfo().GetShape();
    return out;
}
...
inline std::vector<std::vector<int64_t>> Session::GetOverridableInitializerShapes() const {
    ...
    for (size_t i = 0; i < init_count; i++)
        out[i] = GetOverridableInitializerTypeInfo(i).GetTensorTypeAndShapeInfo().GetShape();
    return out;
}

template <typename T>
inline Ort::Value Value::CreateTensor(T* p_data, size_t p_data_element_count,
                                      const std::vector<int64_t>& shape) {
    return CreateTensor(p_data, p_data_element_count * sizeof(T), shape, TypeToTensorType<T>::type);
}

inline Ort::Value Value::CreateTensor(void* p_data, size_t p_data_byte_count,
                                      const std::vector<int64_t>& shape,
                                      ONNXTensorElementDataType type) {
    Ort::MemoryInfo info = Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault);
    return Ort::Value::CreateTensor(info, p_data, p_data_byte_count, shape.data(), shape.size(),
                                    type);
}
