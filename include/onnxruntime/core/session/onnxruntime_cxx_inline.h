...
// If interested in trying out features of the new experimental C++ API, include
// "experimental_onnxruntime_cxx_api.h" instead.
//
// These are the inline implementations of the C++ header APIs. They're in this separate file as to
// not clutter the main C++ file with implementation details.
...
inline MemoryAllocation::MemoryAllocation(MemoryAllocation&& o) noexcept
...
inline MemoryInfo::MemoryInfo(const char* name, OrtAllocatorType type, int id,
                              OrtMemType mem_type) {
...
inline void Allocator::Free(void* p) const { ThrowOnError(GetApi().AllocatorFree(p_, p)); }
...
inline void IoBinding::ClearBoundInputs() { GetApi().ClearBoundInputs(p_); }

inline void IoBinding::ClearBoundOutputs() { GetApi().ClearBoundOutputs(p_); }

inline void IoBinding::SynchronizeInputs() { ThrowOnError(GetApi().SynchronizeBoundInputs(p_)); }

inline void IoBinding::SynchronizeOutputs() { ThrowOnError(GetApi().SynchronizeBoundOutputs(p_)); }
...
inline Env::Env(OrtLoggingLevel logging_level, _In_ const char* logid) {
    ...
    if (strcmp(logid, "onnxruntime-node") == 0) {
        ThrowOnError(
            GetApi().SetLanguageProjection(p_, OrtLanguageProjection::ORT_PROJECTION_NODEJS));
    } else {
        ThrowOnError(
            GetApi().SetLanguageProjection(p_, OrtLanguageProjection::ORT_PROJECTION_CPLUSPLUS));
    }
}

inline Env::Env(OrtLoggingLevel logging_level, const char* logid,
                OrtLoggingFunction logging_function, void* logger_param) {
    ThrowOnError(GetApi().CreateEnvWithCustomLogger(logging_function, logger_param, logging_level,
                                                    logid, &p_));
    if (strcmp(logid, "onnxruntime-node") == 0) {
        ThrowOnError(
            GetApi().SetLanguageProjection(p_, OrtLanguageProjection::ORT_PROJECTION_NODEJS));
...
inline Env::Env(const OrtThreadingOptions* tp_options, OrtLoggingLevel logging_level,
...
inline Env::Env(const OrtThreadingOptions* tp_options, OrtLoggingFunction logging_function,
                void* logger_param, OrtLoggingLevel logging_level, _In_ const char* logid) {
    ThrowOnError(GetApi().CreateEnvWithCustomLoggerAndGlobalThreadPools(
        logging_function, logger_param, logging_level, logid, tp_options, &p_));
    if (strcmp(logid, "onnxruntime-node") == 0) {
        ThrowOnError(
            GetApi().SetLanguageProjection(p_, OrtLanguageProjection::ORT_PROJECTION_NODEJS));
    } else {
...
inline Env& Env::CreateAndRegisterAllocator(const OrtMemoryInfo* mem_info,
...
inline SessionOptions& SessionOptions::SetGraphOptimizationLevel(
    GraphOptimizationLevel graph_optimization_level) {
    ThrowOnError(GetApi().SetSessionGraphOptimizationLevel(p_, graph_optimization_level));
    return *this;
}

inline SessionOptions& SessionOptions::SetOptimizedModelFilePath(
...
inline SessionOptions& SessionOptions::AddConfigEntry(const char* config_key,
...
inline SessionOptions& SessionOptions::AddExternalInitializers(
    ...
    if (inputs_num != ort_values.size()) {
        ORT_CXX_API_THROW("Expecting names and ort_values to have the same length",
    ...
    ThrowOnError(
        GetApi().AddExternalInitializers(p_, names_ptr.data(), ort_values_ptrs.data(), inputs_num));
    return *this;
}

inline SessionOptions& SessionOptions::AppendExecutionProvider_CUDA(
...
inline SessionOptions& SessionOptions::AppendExecutionProvider_CUDA_V2(
...
inline SessionOptions& SessionOptions::AppendExecutionProvider_ROCM(
...
inline SessionOptions& SessionOptions::AppendExecutionProvider_TensorRT_V2(
...
inline SessionOptions& SessionOptions::AppendExecutionProvider_MIGraphX(
...
inline SessionOptions& SessionOptions::AppendExecutionProvider(
    ...
    ThrowOnError(GetApi().SessionOptionsAppendExecutionProvider(
        p_, provider_name.c_str(), keys.data(), values.data(), num_entries));

    return *this;
}

inline SessionOptions& SessionOptions::SetCustomCreateThreadFn(
...
inline SessionOptions& SessionOptions::SetCustomThreadCreationOptions(
...
inline SessionOptions& SessionOptions::SetCustomJoinThreadFn(
...
inline Session::Session(Env& env, const ORTCHAR_T* model_path, const SessionOptions& options,
    ...
    ThrowOnError(GetApi().CreateSessionWithPrepackedWeightsContainer(
        env, model_path, options, prepacked_weights_container, &p_));
}
...
inline std::vector<Value> Session::Run(const RunOptions& run_options,
                                       const char* const* input_names, const Value* input_values,
    ...
    Run(run_options, input_names, input_values, input_count, output_names, output_values.data(),
        output_names_count);
    return output_values;
}

inline void Session::Run(const RunOptions& run_options, const char* const* input_names,
                         const Value* input_values, size_t input_count,
                         const char* const* output_names, Value* output_values,
                         size_t output_count) {
    static_assert(
...
inline AllocatedStringPtr Session::GetInputNameAllocated(size_t index,
...
inline AllocatedStringPtr Session::GetOutputNameAllocated(size_t index,
                                                          OrtAllocator* allocator) const {
    char* out;
    ThrowOnError(GetApi().SessionGetOutputName(p_, index, allocator, &out));
    return AllocatedStringPtr(out, detail::AllocatedFree(allocator));
...
inline AllocatedStringPtr Session::GetOverridableInitializerNameAllocated(
    size_t index, OrtAllocator* allocator) const {
    char* out;
    ThrowOnError(GetApi().SessionGetOverridableInitializerName(p_, index, allocator, &out));
    return AllocatedStringPtr(out, detail::AllocatedFree(allocator));
}
...
inline AllocatedStringPtr Ort::ModelMetadata::GetDescriptionAllocated(
    OrtAllocator* allocator) const {
    char* out;
    ThrowOnError(GetApi().ModelMetadataGetDescription(p_, allocator, &out));
    return AllocatedStringPtr(out, detail::AllocatedFree(allocator));
...
inline AllocatedStringPtr ModelMetadata::GetGraphDescriptionAllocated(
    OrtAllocator* allocator) const {
    char* out;
    ThrowOnError(GetApi().ModelMetadataGetGraphDescription(p_, allocator, &out));
    return AllocatedStringPtr(out, detail::AllocatedFree(allocator));
...
inline char* ModelMetadata::LookupCustomMetadataMap(const char* key,
                                                    OrtAllocator* allocator) const {
    char* out;
...
inline AllocatedStringPtr ModelMetadata::LookupCustomMetadataMapAllocated(
    const char* key, OrtAllocator* allocator) const {
    char* out;
    ThrowOnError(GetApi().ModelMetadataLookupCustomMetadataMap(p_, allocator, key, &out));
    return AllocatedStringPtr(out, detail::AllocatedFree(allocator));
}

inline char** ModelMetadata::GetCustomMetadataMapKeys(OrtAllocator* allocator,
                                                      _Out_ int64_t& num_keys) const {
    char** out;
    ThrowOnError(GetApi().ModelMetadataGetCustomMetadataMapKeys(p_, allocator, &out, &num_keys));
    return out;
}

inline std::vector<AllocatedStringPtr> ModelMetadata::GetCustomMetadataMapKeysAllocated(
    OrtAllocator* allocator) const {
    ...
    // reserve may throw
    auto strings_deletor = [&deletor, num_keys](char** out) {
...
inline void TensorTypeAndShapeInfo::GetSymbolicDimensions(const char** values,
...
inline Value Value::CreateTensor(const OrtMemoryInfo* info, T* p_data, size_t p_data_element_count,
    ...
    return CreateTensor(info, p_data, p_data_element_count * sizeof(T), shape, shape_len,
                        TypeToTensorType<T>::type);
}
...
inline Value Value::CreateTensor(const OrtMemoryInfo* info, void* p_data, size_t p_data_byte_count,
    ...
    OrtValue* out;
    ThrowOnError(GetApi().CreateTensorWithDataAsOrtValue(info, p_data, p_data_byte_count, shape,
                                                         shape_len, type, &out));
    return Value{out};
}
...
template <typename T>
inline Value Value::CreateSparseTensor(const OrtMemoryInfo* info, T* p_data,
...
inline Value Value::CreateSparseTensor(const OrtMemoryInfo* info, void* p_data,
                                       const Shape& dense_shape, const Shape& values_shape,
                                       ONNXTensorElementDataType type) {
    OrtValue* out;
    ThrowOnError(GetApi().CreateSparseTensorWithValuesAsOrtValue(
        info, p_data, dense_shape.shape, dense_shape.shape_len, values_shape.shape,
        values_shape.shape_len, type, &out));
    return Value{out};
}

inline void Value::FillSparseTensorCoo(const OrtMemoryInfo* mem_info,
    ...
    ThrowOnError(GetApi().FillSparseTensorCoo(p_, mem_info, values_param.values_shape,
                                              values_param.values_shape_len,
                                              values_param.data.p_data, indices_data, indices_num));
}
...
inline void Value::FillSparseTensorCsr(const OrtMemoryInfo* data_mem_info,
    ...
    ThrowOnError(GetApi().FillSparseTensorCsr(
        p_, data_mem_info, values.values_shape, values.values_shape_len, values.data.p_data,
        inner_indices_data, inner_indices_num, outer_indices_data, outer_indices_num));
}
...
inline void Value::FillSparseTensorBlockSparse(const OrtMemoryInfo* data_mem_info,
    ...
    ThrowOnError(GetApi().FillSparseTensorBlockSparse(
        p_, data_mem_info, values.values_shape, values.values_shape_len, values.data.p_data,
        indices_shape.shape, indices_shape.shape_len, indices_data));
}
...
inline void Value::UseCsrIndices(int64_t* inner_data, size_t inner_num, int64_t* outer_data,
                                 size_t outer_num) {
...
inline void Value::UseBlockSparseIndices(const Shape& indices_shape, int32_t* indices_data) {
    ThrowOnError(GetApi().UseBlockSparseIndices(p_, indices_shape.shape, indices_shape.shape_len,
                                                indices_data));
}
...
inline TensorTypeAndShapeInfo Value::GetSparseTensorIndicesTypeShapeInfo(
    OrtSparseIndicesFormat indices_format) const {
    OrtTensorTypeAndShapeInfo* output;
...
template <typename T>
inline const T* Value::GetSparseTensorIndicesData(OrtSparseIndicesFormat indices_format,
...
inline Value Value::CreateSparseTensor(OrtAllocator* allocator, const Shape& dense_shape,
                                       ONNXTensorElementDataType type) {
    OrtValue* out;
    ThrowOnError(GetApi().CreateSparseTensorAsOrtValue(allocator, dense_shape.shape,
                                                       dense_shape.shape_len, type, &out));
    return Value{out};
}
#endif  // !defined(DISABLE_SPARSE_TENSORS)
...
template <typename T>
inline Value Value::CreateOpaque(const char* domain, const char* type_name,
                                 const T& data_container) {
    OrtValue* out;
...
inline void Value::GetStringTensorContent(void* buffer, size_t buffer_length, size_t* offsets,
    ...
    ThrowOnError(
        GetApi().GetStringTensorContent(p_, buffer, buffer_length, offsets, offsets_count));
}
...
template <>
inline float CustomOpApi::KernelInfoGetAttribute<float>(_In_ const OrtKernelInfo* info,
...
template <>
inline int64_t CustomOpApi::KernelInfoGetAttribute<int64_t>(_In_ const OrtKernelInfo* info,
                                                            _In_ const char* name) {
    int64_t out;
...
template <>
inline std::vector<float> CustomOpApi::KernelInfoGetAttribute(_In_ const OrtKernelInfo* info,
                                                              _In_ const char* name) {
    size_t size = 0;
...
template <>
inline std::vector<int64_t> CustomOpApi::KernelInfoGetAttribute(_In_ const OrtKernelInfo* info,
                                                                _In_ const char* name) {
    size_t size = 0;
...
inline ONNXTensorElementDataType CustomOpApi::GetTensorElementType(
...
inline void CustomOpApi::GetDimensions(_In_ const OrtTensorTypeAndShapeInfo* info,
                                       _Out_ int64_t* dim_values, size_t dim_values_length) {
    ThrowOnError(api_.GetDimensions(info, dim_values, dim_values_length));
}

inline void CustomOpApi::SetDimensions(OrtTensorTypeAndShapeInfo* info,
...
inline const OrtValue* CustomOpApi::KernelContext_GetInput(const OrtKernelContext* context,
                                                           _In_ size_t index) {
    const OrtValue* out;
...
inline OrtOpAttr* CustomOpApi::CreateOpAttr(_In_ const char* name, _In_ const void* data,
...
inline OrtOp* CustomOpApi::CreateOp(
    _In_ const OrtKernelInfo* info, _In_ const char* op_name, _In_ const char* domain,
    _In_ int version, _In_opt_ const char** type_constraint_names,
    _In_opt_ const ONNXTensorElementDataType* type_constraint_values,
    _In_opt_ int type_constraint_count, _In_opt_ const OrtOpAttr* const* attr_values,
    _In_opt_ int attr_count, _In_ int input_count, _In_ int output_count) {
    OrtOp* ort_op{};
    ThrowOnError(api_.CreateOp(info, op_name, domain, version, type_constraint_names,
                               type_constraint_values, type_constraint_count, attr_values,
                               attr_count, input_count, output_count, &ort_op));
    return ort_op;
}

inline void CustomOpApi::InvokeOp(_In_ const OrtKernelContext* context, _In_ const OrtOp* ort_op,
                                  _In_ const OrtValue* const* input_values, _In_ int input_count,
                                  _Inout_ OrtValue* const* output_values, _In_ int output_count) {
    ThrowOnError(
        api_.InvokeOp(context, ort_op, input_values, input_count, output_values, output_count));
}

inline void CustomOpApi::ReleaseOp(_Frees_ptr_opt_ OrtOp* ort_op) { api_.ReleaseOp(ort_op); }
